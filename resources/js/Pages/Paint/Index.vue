...
const updId = ref();
const updTitle = ref();
const updConsumption = ref();

const columnHeaders = [
    '№', 'Наименование краски', 'Расход краски', 'Редактирование', 'Удаление'
];
...
const showUpdate = (item) => {
    ...
    form.consumption = item.consumption
}

const form = useForm({
    title: updTitle.value,
    consumption: updConsumption.value
})
const submit = () => {
    form.put(route('update.paint', updId.value))
    closeUpdate()
}
const closeUpdate = () => {
    hideUpdate.value = !hideUpdate.value
            ...
            <table class="relative w-full border mb-3 table-fixed">
                ...
                <tr v-for="(item, index) in paints">
                    ...
                    <td :class='["px-6 py-2 text-center", index%2 === 0 ? "" : "bg-gray-300"]'>
                        ...
                        <div v-if="!hideUpdate && item.id === updId">
                            <input v-model="form.consumption" id="updPosition" class="h-8 bg-gray-50 border border-gray-600
                            text-gray-900 text-sm italic text-center rounded-lg focus:ring-blue-500 focus:border-blue-500
                            w-full block text-xs">
                        </div>
                    </td>
                    <td :class='["px-2 py-1.5 text-center", index%2 === 0 ? "" : "bg-gray-300"]'>
                        ...
                        <div v-if="!hideUpdate && item.id === updId" class="flex justify-center">
                            <svg @click.prevent="submit" xmlns="http://www.w3.org/2000/svg"
                                 ...
                                 :class="['w-6 h-6 cursor-pointer', { 'opacity-25': form.processing }]">
                    ...
                    <td :class='["px-6 py-2 text-center", index%2 === 0 ? "" : "bg-gray-300"]'>
                        <DeleteButton :item="item" :path="'paint'"></DeleteButton>
                    </td>
                </tr>
                </tbody>
            </table>
            <StoreComponent :hide-material="hideMaterial" :element="'paint'" :element-title="'краски'"
