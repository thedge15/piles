...
const columnHeaders = ['Наименование металлопроката', 'Посмотреть металлопрокат', 'Редактирование', 'Удаление']
...
const submit = () => {
    form.put(route('update.metal', updId.value))
    closeUpdate()
}
const closeUpdate = () => {
    hideUpdate.value = !hideUpdate.value
        ...
        <div class="flex-grow overflow-auto">
            <table class="relative w-full border mb-3 table-fixed">
                ...
                <tr v-for="(item, index) in metals">
                    ...
                    <td :class='["px-6 py-2 text-center", index%2 === 0 ? "" : "bg-gray-300"]'>
                        <DeleteButton :item="item" :path="'metal'"></DeleteButton>
                    </td>
                </tr>
                </tbody>
            </table>
            <StoreComponent :hide-material="hideMaterial" :element="'metal'" :element-title="'металлопроката'"
