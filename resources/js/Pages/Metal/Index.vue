<template>
    ...
    <div class="flex-grow overflow-auto">
            ...
            <tr v-for="(item, index) in metals">
                ...
                <td :class='["px-2 py-1.5 text-center", index%2 === 0 ? "" : "bg-gray-300"]'>
                    ...
                    <div v-if="!this.hideUpdate && item.id === this.updId" class="flex justify-center">
                        <svg @click.prevent="updateProject('metal')" xmlns="http://www.w3.org/2000/svg" fill="none"
        ...
        <DeleteComponent :del-element="this.delElement" :hide-delete="this.hideDelete" :del-title="'metal'"
                         @closeDelete="closeDelete"></DeleteComponent>
        <StoreComponent :hide-material="this.hideMaterial" :element="'metal'" :element-title="'металлопроката'"
...
export default {
    ...
    data() {
        return {
            columnHeaders: ['Наименование металлопроката', 'Посмотреть металлопрокат', 'Редактирование', 'Удаление'],
        }
    },
