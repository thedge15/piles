...
const submit = () => {
    form.put(route('update.element', updId.value))
    closeUpdate()
}
const closeUpdate = () => {
    hideUpdate.value = !hideUpdate.value
        ...
        <div class="flex-grow overflow-auto">
            <table class="relative w-full border mb-3 text-xs table-fixed">
                ...
                <tr v-for="(item, index) in elements">
                    ...
                    <td :class='["px-6 py-1 text-center", index%2 === 0 ? "" : "bg-gray-300"]'>
                        <DeleteButton :item="item" :path="'element'"></DeleteButton>
                    </td>
                </tr>
                </tbody>
            </table>
        </div>
        <StoreComponent :hide-material="hideMaterial" :element-title="'элемента'"
                        :element="'element'" @closeStore="closeStore"></StoreComponent>
    </UserLayout>
</template>
